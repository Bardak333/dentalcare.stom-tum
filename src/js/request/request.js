...
class Request {
    ...
    setupEventListeners() {
        const submitBtn = document.querySelector('.request__block--btn');
        submitBtn.addEventListener('click', () => this.createRequest());

        const toggle = document.querySelector('.request__form--toggle');
        toggle.addEventListener('click', () => this.toggleAgreement());

        const nameInput = document.querySelector('.request__form--input[type="text"]');
        const phoneInput = document.querySelector('.request__form--input[type="tel"]');
    ...
    toggleAgreement() {
        ...
        const toggle = document.querySelector('.request__form--toggle');
        const btn = document.querySelector('.request__block--btn');
    ...
    createRequest() {
        if (this.isAgree) {
            ...
            if (Object.keys(newErrors).length === 0) {
                ...
                // Сброс UI
                this.clearAllErrors();
                document.querySelector('.request__form--toggle').classList.add('active');
            }
        }
    }
    ...
    updateErrorsUI() {
        ...
        if (this.errors.name) {
            const nameForm = document.querySelector('.request__form--item1');
            nameForm.classList.add('unapprecial');
            this.nameInput.placeholder = this.errors.name;
        }

        if (this.errors.phone) {
            const phoneForm = document.querySelector('.request__form--item2');
    ...
    clearAgreeError() {
        const agreeToggle = document.querySelector('.request__form--toggle');
